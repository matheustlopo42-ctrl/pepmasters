...
app.post('/api/admin/pedido-manual', adminMiddleware, async (req, res) => {
  ...
  // Calcular split com variação de centavos para evitar bloqueio bancário
  ...
  try {
      ...
      for (let i = 0; i < numQrs; i++) {
        ...
        try {
          ...
          if (pd.success && pd.data) {
            qrs.push({ parte: i+1, valor, qrcode_url: pd.data.qr_image_url||null, copia_cola: pd.data.qr_code||null, payment_id: pd.data.payment_id||pd.data.id||null });
    ...
    enviarWhatsApp(`📋 Pedido manual #${pedidoId} — ${nome}${telefone?' | '+telefone:''} — ${produto_nome} — R$ ${valorTotal.toFixed(2).replace('.',',')} — ${numQrs} QR(s)`);
    ...
    res.json({
      ...
      whatsapp_link: telefone
        ? `https://wa.me/${telefone.replace(/\D/g,'')}?text=${encodeURIComponent(
            `Olá ${nome.split(' ')[0]}! Pedido PEPMASTERS #${pedidoId}.\n*Produto:* ${produto_nome}\n*Total: R$ ${valorTotal.toFixed(2).replace('.',',')}*\n` +
