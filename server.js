    ...
    if (pedido.email_cliente) {
      enviarEmail(pedido.email_cliente,
        ...
        `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          ...
          <p><strong>Produto:</strong> ${pedido.produto_nome}<br><strong>Total:</strong> R$ ${parseFloat(pedido.total).toFixed(2).replace('.',',')}</p>
          <p>O fornecedor foi notificado e em breve enviará seu produto.</p>
        </div>`
      ).catch(()=>{});
    }
    ...
    if (fornR.rows.length) {
      ...
      enviarEmail(forn.email,
        ...
        `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          ...
          <strong>Quantidade:</strong> ${pedido.quantidade}<br><strong>Cliente:</strong> ${pedido.nome_cliente}<br>
    ...
    if (pedido.email_cliente) {
      enviarEmail(pedido.email_cliente,
        ...
        `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          ...
          <p>Olá, <strong>${(pedido.nome_cliente||'cliente').split(' ')[0]}</strong>!</p>
          <p>Seu pedido <strong>#${pedido.id}</strong> de <strong>${pedido.produto_nome}</strong> foi enviado.</p>
          ${pedido.codigo_rastreio?`<p><strong>Rastreio:</strong> ${pedido.codigo_rastreio}</p>`:''}
          <p>⏳ O pagamento é liberado automaticamente em <strong>5 dias</strong> sem disputa.</p>
          <p>Após receber, avalie o fornecedor em <a href="${BASE_URL}/club-meus-pedidos.html">Meus Pedidos</a>. ⭐</p>
        </div>`
      ).catch(()=>{});
    }
...
// GET /api/club/avaliacoes/:fornecedor_id
...
// GET /api/club/fornecedor/avaliacoes
...
// PUT /api/club/fornecedor/avaliacao/:id/responder
